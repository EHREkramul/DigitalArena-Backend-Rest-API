...
export class ReviewsService {
    ...
    async addReview(createReviewDto: CreateReviewDto): Promise<Review> {
        const { userId, productId, rating, comment, reviewStatus } = createReviewDto;
    ...
    async changeReviewStatus(reviewId: number, status: number): Promise<Review> {
        ...
        if (status !== 1 && status !== 0) {
            throw new BadRequestException('Invalid status value. Use 1 for ACCEPTED and 0 for REJECTED.');
        }
        ...
        review.reviewStatus = status === 1 ? ReviewStatus.APPROVED : ReviewStatus.REJECTED;
