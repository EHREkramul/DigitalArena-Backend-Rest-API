...
export class AdminStatisticsService {
    ...
    async getSellingTimeline(
        ...
        return await this.orderRepository.query(query, [startDate, endDate, granularity]);
    }
        ...
        try {
            ...
            if (startDate) {
                ...
                if (isNaN(start.getTime())) {
                    return { totalRevenue: 0, message: 'Invalid startDate format. Use YYYY-MM-DD.' };
                }
            }
            if (endDate) {
                end = new Date(endDate);
                if (isNaN(end.getTime())) {
                    return { totalRevenue: 0, message: 'Invalid endDate format. Use YYYY-MM-DD.' };
                }
            }
